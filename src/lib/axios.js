...
import {
  buildChannelAuthErrorUrl,
  clearChannelTokenInUrl,
  getEffectiveChannelToken,
  isChannelLogin,
  saveChannelReturnUrl,
  saveChannelToken,
} from '/@/utils/channel-login-util';
...
function redirectChannelAuthError() {
  useUserStore().logout({ keepReturnUrl: true });
  location.href = buildChannelAuthErrorUrl({ reason: 'expired' });
}

async function silentChannelLogin() {
  const channelToken = getEffectiveChannelToken();
  if (!channelToken) {
    return false;
  }
  try {
    const result = await smartAxios.request({
      url: '/login/channelLogin',
      method: 'get',
      params: { channelToken },
      __skipAuth: true,
    });
    ...
    saveChannelToken(channelToken);
    saveChannelReturnUrl(result?.data?.channelReturnUrl || '');
    clearChannelTokenInUrl();
    return true;
  } catch (e) {
    return false;
...
async function loadChannelMeta(channelToken) {
  if (!channelToken) {
    return;
  }
  try {
    const result = await smartAxios.request({
      url: '/login/channelMeta',
      method: 'get',
      params: { channelToken },
      __skipAuth: true,
    });
    saveChannelReturnUrl(result?.data?.data?.returnUrl || result?.data?.returnUrl || '');
  } catch (e) {
    saveChannelReturnUrl('');
  }
}
  ...
  async (response) => {
    ...
    if (res.code && res.code !== 1) {
      ...
      if (res.code === 30007 || res.code === 30008) {
        const isChannelReq = response.config?.url?.includes('/login/channelLogin');
        const hasRetryFlag = response.config && response.config.__channelReloginRetry;
        if (isChannelLogin() && !hasRetryFlag && !isChannelReq) {
          const reloginSuccess = await silentChannelLogin();
          if (reloginSuccess) {
            const retryConfig = {
              ...response.config,
              __channelReloginRetry: true,
            };
            return smartAxios.request(retryConfig);
          }
          message.destroy();
          message.error('渠道登录已失效，请从业务平台重新进入');
          await loadChannelMeta(getEffectiveChannelToken());
          setTimeout(redirectChannelAuthError, 300);
 ...
 * 文件下载
 */
export const getDownload = function (url, params, fun) {
  request({
    method: 'get',
    url,
    params,
    ...
    .then((data) => {
      handleDownloadData(data, fun);
    })
    .catch((error) => {
      handleDownloadError(error, fun);
    });
};

function handleDownloadError(error, fun) {
...
function handleDownloadData(response, fun) {
  if (fun) {
    fun();
  }
