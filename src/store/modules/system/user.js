...
import { clearChannelLogin } from '/@/utils/channel-login-util.js';

const MAX_KEEP_ALIVE_INCLUDES = 100;
...
export const useUserStore = defineStore({
  ...
  actions: {
    logout(option = {}) {
      ...
      clearChannelLogin(option);

      // 断开 WebSocket 连接并重置消息状态
    ...
    pushKeepAliveIncludes(val) {
      ...
      let index = this.keepAliveIncludes.findIndex((e) => e === val);
      if (index !== -1) {
      ...
      if (this.keepAliveIncludes.length > MAX_KEEP_ALIVE_INCLUDES) {
        this.keepAliveIncludes.splice(0, this.keepAliveIncludes.length - MAX_KEEP_ALIVE_INCLUDES);
      }
    },
    // 删除缓存
