        ...
        <div class="tree-container">
          ...
          >
            <template #title="{ indicatorName, indicatorCode }">
              <span>{{ indicatorName }} <span class="indicator-code">({{ indicatorCode }})</span></span>
            </template>
          </a-tree>
        </div>
      </a-spin>
...
<script setup>
  ...
  import { roleIndicatorApi } from '/@/api/system/role-indicator-api';
  ...
  async function loadRoleIndicatorPermissions() {
    ...
    try {
      // 调用API获取角色指标权限
      const res = await roleIndicatorApi.queryByRoleId(selectRoleId.value);
  ...
  async function loadFullIndicatorTree() {
    try {
      const res = await indicatorManageApi.queryIndicatorTree({});
      if (res.ok && res.data) {
        // 构建树形表格数据
      ...
      if (isConfigured) {
        ...
        if (node.children && node.children.length > 0) {
          const childrenNodes = node.children
            .map(child => processNode(child, currentPath, depth + 1))
            .filter(Boolean);
  ...
  async function handleSavePermissions() {
    ...
    try {
      ...
      // // 调用保存API
      const res = await roleIndicatorApi.saveRoleIndicator(saveData);
  ...
  async function handleDelete(id) {
    ...
    try {
      // 调用删除API
      const res = await roleIndicatorApi.delete(id);
