         ...
         <a-modal v-model:open="modalOpen" :title="modalObj.title" :footer="null" :maskClosable="false" :width="modalObj.width">
            <div class="model_info" v-if="modalObj.type=='form'">
                <a-spin :spinning="spinning" tip="操作中...">
                    <FormIndex v-model:formData="formData" :formItemList="formItemList" :formProps="formProps" @onSubmit="onSubmit" @closeModalFun="closeModalFun"/>
                </a-spin>
            </div> 
...
import { formTitle,columns,formItemList,init_data } from './tableColumns';
...
const formProps=ref({isModal:true,isView:false,cell:12});
//创建表单
const addForm=()=>{
    init_data(formData,{});
    openModalFun({title:'创建'+formTitle,type:'form',width:'60%'});
}
//表单提交
const onSubmit=()=>{   
    spinning.value=true;   
    IndicatorApi.add(formData).then((res)=>{
        if(res.code==0){
...
const deleteFun=(item)=>{
    if(item.k_complete_value>0){
        Modal.warning({
            title: '删除提示',
            content: formTitle+':【'+item.k_name+'】已有完成进度，无法进行删除！',
        });
        return;
    }
    Modal.confirm({
        ...
        content: '是否要删除'+formTitle+':【'+item.k_name+'】？删除后将无法恢复！',
        okText: '确认',
        cancelText: '取消',
        onOk() {
            IndicatorApi.del(item.id).then((res)=>{
              if(res.code==0){
                    message.success('操作成功！');
                    queryData();                   
                }        
...
const columnClick=(item,lx,index)=>{
    switch(lx){
        case '1':
            init_data(formData,item);
            openModalFun({title:'编辑'+formTitle+'：'+item.k_name,type:'form',width:'60%'})
            break;//编辑
        case '-1': 
            deleteFun(item);
            break;//删除
