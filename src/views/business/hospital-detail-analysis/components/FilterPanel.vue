...
const filterParams = ref({
  ...
  hospitalName: '',
  diseaseCode: '',
  diseaseType: ''  // 病种类型: category(类别), item(类目), subitem(亚目)
});
...
// Event handlers
const handleSearch = () => {
  // 将日期对象转换为字符串格式,并转换为完整日期
  const startDateStr = filterParams.value.startDate?.format('YYYY-MM');
  const endDateStr = filterParams.value.endDate?.format('YYYY-MM');
  
  const searchParams = {
    ...filterParams.value,
    startDate: startDateStr ? `${startDateStr}-01` : undefined,
    endDate: endDateStr ? `${endDateStr}-31` : undefined
  };
...
const handleReset = () => {
  filterParams.value = {
    startDate: dayjs('2023-12'),
    endDate: dayjs('2023-12'),
    districtId: '',
    town: '',
    insuranceType: '',
    hospitalLevel: '',
...
// 处理病种选择变化
const handleDiseaseChange = (nodeInfo) => {
  if (nodeInfo) {
    filterParams.value.diseaseCode = nodeInfo.code || nodeInfo.value;
    filterParams.value.diseaseType = nodeInfo.type || '';
    console.log('病种选择变化:', {
      code: filterParams.value.diseaseCode,
      type: filterParams.value.diseaseType,
      name: nodeInfo.name
    });
  } else {
    // 清除选择
    filterParams.value.diseaseCode = '';
    filterParams.value.diseaseType = '';
  }
};
</script>
