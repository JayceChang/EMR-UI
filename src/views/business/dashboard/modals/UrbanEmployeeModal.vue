      ...
      <a-spin :spinning="loading">
        ...
        <div class="chart-row">
          <div class="chart-container">
            <h4>各县区城镇职工参保人数与医保资金</h4>
            <div ref="barLineChartRef" class="chart"></div>
          </div>
          <div class="chart-container">
            <h4>各县区城镇职工参保人数与医保资金历年对比</h4>
...
<script setup>
  ...
  const modalTitle = ref('城镇职工医保数据下钻明细');
  ...
  const loadData = async () => {
    loading.value = true;
    try {
      const response = await DrillDownAPI.getUrbanEmployeeDetail({
        ...
        series: [
          {
            name: '参保人数',
            type: 'bar',
            data: (props.districtData || []).map((d) => d.employeeCount),
          },
          {
            name: '医保资金',
            type: 'line',
            yAxisIndex: 1,
            data: (props.districtData || []).map((d) => d.employeeFund),
          },
        ],
      });
    }
