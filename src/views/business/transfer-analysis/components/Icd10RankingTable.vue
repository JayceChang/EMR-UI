<template>
  <div class="overflow-x-auto">
    <a-table
      ...
      size="small"
      class="min-w-full"
      :row-key="(record) => record.category"
    >
      <template #bodyCell="{ column, record }">
        <template v-if="column.key === 'category'">
          <div class="text-sm font-medium text-gray-900">{{ record.category }}</div>
          <div class="text-sm font-medium text-gray-900">{{ record.categoryName }}</div>
        </template>
...
<script setup>
  ...
  const handleDrillDown = (record) => {
    emit('drill-down', {
      scene: 'disease',
      title: `病种下钻 - ${record.categoryName || record.category}`,
      diseaseCode: record.category,
      diseaseName: record.categoryName,
      sourceRecord: record,
    });
  };
</script>
