  ...
  <div class="bg-white rounded-xl p-5 shadow-md overflow-hidden">
    <div class="flex flex-col sm:flex-row justify-between items-start sm:items-center mb-4">
      <h3 class="text-lg font-semibold flex items-center">
        <i class="fas fa-disease text-blue-600 mr-2"></i>
        住院数据按病种汇总
      </h3>
      <div class="flex flex-wrap gap-2 mt-3 sm:mt-0">
        <a-input-search v-model:value="searchText" placeholder="搜索病种..." style="width: 200px" @search="handleSearch" />
      </div>
    </div>
    ...
    <a-table
      ...
      :loading="loading"
      size="small"
      :row-key="(record) => record.diseaseCode"
      :scroll="{ x: 1100 }"
      @change="handleTableChange"
    >
      <template #bodyCell="{ column, record }">
        <template v-if="column.key === 'diseaseName'">
          <div class="cursor-pointer" @click="handleRowClick(record)">
            <p class="font-medium">{{ record.diseaseName }}</p>
            <p class="text-xs text-gray-500">{{ record.diseaseCode }}</p>
          </div>
        </template>
...
<script setup>
  ...
  const columns = [
    { title: '病种名称', dataIndex: 'diseaseName', key: 'diseaseName', width: 200, sorter: true },
    ...
    { title: '次均统筹', dataIndex: 'avgReimbursement', key: 'avgReimbursement', width: 140, sorter: true },
  ...
  const filteredData = computed(() => {
    ...
    return props.data.filter(
      (item) =>
        item.diseaseName.toLowerCase().includes(searchText.value.toLowerCase()) ||
        item.diseaseCode.toLowerCase().includes(searchText.value.toLowerCase())
    );
  });
  ...
  const handleRowClick = (record) => {
    ...
    emit('drill-down', 'disease', record);
  };
