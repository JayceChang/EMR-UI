...
<script setup>
  ...
  import { CLINIC_ANALYSIS_DRILL_DOWN_ENDPOINT, getClinicPatientDrillPage } from '/@/api/business/clinic-analysis/clinic-analysis-api';
  ...
  const modalTitle = computed(() => props.context?.title || '门诊患者明细下钻');
  ...
  const contextDescription = computed(() => {
    ...
    const scopeText = props.context?.institutionName || props.context?.countyName || '当前筛选范围';
    const startMonth = props.context?.startMonth || '--';
    const endMonth = props.context?.endMonth || '--';
    return `来源: ${sourceText} | 范围: ${scopeText} | 时间: ${startMonth} 至 ${endMonth}`;
  });
  ...
  const getRequestParams = (pageNum, size) => {
    ...
    return {
      ...
      countyName: props.context?.countyName,
      institutionName: props.context?.institutionName,
      level: props.context?.level,
      division: props.context?.division,
      keyword,
  ...
  const loadData = async (pageNum = currentPage.value, size = pageSize.value) => {
    ...
    try {
      if (import.meta.env.DEV && CLINIC_ANALYSIS_DRILL_DOWN_ENDPOINT !== '/business/clinic/detail/drill/page') {
        throw new Error(`门诊分析下钻接口异常: ${CLINIC_ANALYSIS_DRILL_DOWN_ENDPOINT}`);
      }
      loading.value = true;
      const response = await getClinicPatientDrillPage(getRequestParams(pageNum, size));
      ...
      console.error('加载门诊下钻明细失败:', error);
      message.error('加载门诊下钻明细失败');
      tableData.value = [];
      total.value = 0;
      buildColumns([]);
    } finally {
