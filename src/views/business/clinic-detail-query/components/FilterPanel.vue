  ...
  <div class="bg-white rounded-xl p-5 filter-card sticky top-6">
    ...
    <a-form class="space-y-2" layout="vertical">
      ...
      <div class="space-y-1">
        ...
        <div class="space-y-1">
          <a-select
            ...
            placeholder="选择开始月份"
          >
            <a-select-option value="2024-07">2024年07月</a-select-option>
      ...
      <a-form-item label="ICD类目">
        <a-select
          v-model:value="filterParams.icdCategory"
          class="w-full"
          placeholder="选择类目"
          @change="handleIcdCategoryChange"
        >
      ...
      <a-form-item label="ICD亚目">
        <a-select
          v-model:value="filterParams.icdSubcategory"
          class="w-full"
          placeholder="选择亚目"
          :disabled="!filterParams.icdCategory"
        >
          <a-select-option value="">全部亚目</a-select-option>
          <a-select-option
            v-for="sub in availableIcdSubs"
            :key="sub.value"
            :value="sub.value"
          >
            {{ sub.text }}
...
// Available ICD subcategories based on selected category
const availableIcdSubs = computed(() => {
  if (props.icdData[filterParams.value.icdCategory]) {
    return props.icdData[filterParams.value.icdCategory];
  }
  return [];
});
...
const handleIcdCategoryChange = (value) => {
  // Reset subcategory when category changes
  filterParams.value.icdSubcategory = '';
};
</script>
